...
class TaskRunner:
    ...
    def add_actor_rollout_worker(self, config):
        ...
        diagnostics_cfg = config.actor_rollout_ref.actor.get("sdpo_diagnostics", {})
        loss_mode = config.actor_rollout_ref.actor.policy_loss.get("loss_mode", "vanilla")
        self_distillation_needs_ref = self_distillation_cfg is not None and (
            loss_mode == "sdpo" or diagnostics_cfg.get("enable", False)
        )
        if self_distillation_needs_ref and need_reference_policy(config):
            raise ValueError("SDPO diagnostics/teacher cannot share the reference policy with KL regularization.")
    ...
    def run(self, config):
        ...
        with open_dict(config.actor_rollout_ref.actor):
            config.actor_rollout_ref.actor.offpolicy_routing = OmegaConf.to_container(
                config.algorithm.offpolicy_routing, resolve=True
            )
            config.actor_rollout_ref.actor.far_correct_sft = OmegaConf.to_container(
                config.algorithm.far_correct_sft, resolve=True
            )
            config.actor_rollout_ref.actor.sdpo_routing = OmegaConf.to_container(
                config.algorithm.sdpo_routing, resolve=True
            )
            config.actor_rollout_ref.actor.kl_routing = OmegaConf.to_container(
                config.algorithm.kl_routing, resolve=True
            )
