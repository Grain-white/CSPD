...
def compute_self_distillation_per_token(
    ...
    sdpo_route_mask: Optional[torch.Tensor] = None,
    rollout_is_weights: Optional[torch.Tensor] = None,
) -> tuple[torch.Tensor, torch.Tensor, dict[str, torch.Tensor]]:
    """Return the exact token loss used by SDPO plus diagnostic tensors.

    Keeping this logic shared with :func:`compute_self_distillation_loss`
    prevents diagnostic formulas from drifting away from the optimized loss.
    """

    loss_mask = response_mask
    if self_distillation_mask is not None:
        loss_mask = loss_mask * self_distillation_mask.unsqueeze(1)
    if sdpo_route_mask is not None:
        loss_mask = loss_mask * sdpo_route_mask.unsqueeze(1)
    ...
    details = {
        ...
        "selected_token_logprob_delta": student_log_probs - teacher_log_probs,
        "loss_mask": loss_mask,
        "per_token_loss": per_token_loss,
    }
    if self_distillation_config.full_logit_distillation:
        details["student_distill_log_probs"] = student_distill_log_probs
        details["teacher_distill_log_probs"] = teacher_distill_log_probs
        has_tail = (
            self_distillation_config.distillation_topk is not None
            and self_distillation_config.distillation_add_tail
        )
        if has_tail:
            details["student_topk_mass"] = student_distill_log_probs[..., :-1].exp().sum(-1)
            details["teacher_mass_on_student_topk"] = teacher_distill_log_probs[..., :-1].exp().sum(-1)
            details["student_tail_mass"] = student_distill_log_probs[..., -1].exp()
            details["teacher_tail_mass"] = teacher_distill_log_probs[..., -1].exp()
        else:
            details["student_topk_mass"] = student_distill_log_probs.exp().sum(-1)
            details["teacher_mass_on_student_topk"] = teacher_distill_log_probs.exp().sum(-1)
            details["student_tail_mass"] = torch.zeros_like(per_token_loss)
            details["teacher_tail_mass"] = torch.zeros_like(per_token_loss)
    return per_token_loss, loss_mask, details
...
def compute_self_distillation_loss(
    ...
    metrics = {}
    per_token_loss, loss_mask, details = compute_self_distillation_per_token(
        student_log_probs=student_log_probs,
        teacher_log_probs=teacher_log_probs,
    ...
    if sdpo_route_mask is not None:
        metrics["self_distillation/routed_sample_fraction"] = (sdpo_route_mask > 0).float().mean().detach().item()
        metrics["self_distillation/routed_token_count"] = loss_mask.sum().detach().item()
    if return_details:
...
@register_policy_loss("cspd")
def compute_policy_loss_cspd(
    old_log_prob: torch.Tensor,
    log_prob: torch.Tensor,
    advantages: torch.Tensor,
    response_mask: torch.Tensor,
    ...
    cspd_target_probs: torch.Tensor | None = None,
    cspd_value_weights: torch.Tensor | None = None,
    cspd_selected_mask: torch.Tensor | None = None,
) -> tuple[torch.Tensor, dict[str, Any]]:
    """Frozen value-weighted success-posterior forward KL."""
    from verl.trainer.ppo.cspd import cspd_forward_kl

    assert student_topk_log_probs is not None
    assert cspd_target_probs is not None
    assert cspd_value_weights is not None
    assert cspd_selected_mask is not None
    return cspd_forward_kl(
        student_topk_logp=student_topk_log_probs,
        target=cspd_target_probs,
        value_weight=cspd_value_weights,
        selected_mask=cspd_selected_mask,
    )
