...
from verl.utils.reward_score.feedback.math import last_boxed_only_string
...
def _masked_diagnostic_stats(prefix: str, values: torch.Tensor, mask: torch.Tensor) -> dict[str, float]:
    """Population statistics for detached diagnostics on the full rollout batch."""
    selected = values.detach().float()[mask.bool()]
    if selected.numel() == 0:
        return {}
    return {
        f"{prefix}/mean": selected.mean().item(),
        f"{prefix}/std": selected.std(unbiased=False).item(),
        f"{prefix}/rms": selected.square().mean().sqrt().item(),
        f"{prefix}/abs_mean": selected.abs().mean().item(),
        f"{prefix}/min": selected.min().item(),
        f"{prefix}/max": selected.max().item(),
        f"{prefix}/count": float(selected.numel()),
    }
...
class RayPPOTrainer:
    ...
    @staticmethod
    def _cosine_annealed_value(step: int, total_steps: int, start: float, end: float) -> float:
        """Cosine anneal from start→end over [0, total_steps-1] (or total_steps if <=1)."""
        if total_steps <= 1:
            return float(end)
        progress = min(max(step, 0), total_steps - 1) / (total_steps - 1)
        return float(start + (end - start) * 0.5 * (1.0 - math.cos(math.pi * progress)))

    def _truncate_demonstration_random_prefix(self, text: str, k: float) -> tuple[str, int, int]:
        """Keep a random token prefix of length L~Uniform{0,...,floor(k * n)}.

        Returns (truncated_text, kept_tokens, original_tokens).
        """
        token_ids = self.tokenizer.encode(text, add_special_tokens=False)
        n = len(token_ids)
        if n == 0:
            return "", 0, 0
        max_keep = int(math.floor(max(0.0, min(1.0, k)) * n))
        keep = int(np.random.randint(0, max_keep + 1))
        if keep <= 0:
            return "", 0, n
        return self.tokenizer.decode(token_ids[:keep], skip_special_tokens=True), keep, n

    @staticmethod
    def _extract_final_answer_demonstration(text: str) -> str:
        """Keep only the last \\boxed{...} span; fall back to the original text."""
    ...
    def _maybe_build_self_distillation_batch(
        ...
        sdpo_routing_enabled = self.config.algorithm.get("sdpo_routing", {}).get("enable", False)
        diagnostics_cfg = self.config.actor_rollout_ref.actor.get("sdpo_diagnostics", {})
        diagnostics_capture = (
            diagnostics_cfg.get("enable", False)
            and self.global_steps in set(diagnostics_cfg.get("capture_steps", []))
        )
        if self_distillation_cfg is None or (
            loss_mode != "sdpo" and not sdpo_routing_enabled and not diagnostics_capture
        ):
            return None
        ...
        for i in range(batch_size):
            candidates = list(success_by_uid[batch.non_tensor_batch["uid"][i]])
            if self_distillation_cfg.dont_reprompt_on_self_success:
                candidates = [j for j in candidates if j != i]
            solution_indices.append(candidates[0] if candidates else -1)

        demo_mode = self_distillation_cfg.get("demonstration_content_mode", "full")
        prefix_k = None
        if demo_mode == "random_prefix":
            prefix_k = self._cosine_annealed_value(
                self.global_steps,
                self.total_training_steps,
                float(self_distillation_cfg.get("random_prefix_k_start", 0.95)),
                float(self_distillation_cfg.get("random_prefix_k_end", 0.05)),
            )

        solution_strs = []
        demo_orig_token_lens: list[int] = []
        demo_kept_token_lens: list[int] = []
        for solution_idx in solution_indices:
            solution_str = response_texts[solution_idx] if solution_idx >= 0 else None
            if solution_str is not None and self_distillation_cfg.get("remove_thinking_from_demonstration", False):
                solution_str = self._remove_thinking_trace(solution_str)
            if solution_str is not None and demo_mode == "random_prefix":
                solution_str, kept, orig = self._truncate_demonstration_random_prefix(solution_str, prefix_k)
                demo_kept_token_lens.append(kept)
                demo_orig_token_lens.append(orig)
            elif solution_str is not None and demo_mode == "final_answer":
                solution_str = self._extract_final_answer_demonstration(solution_str)
        ...
        if demo_mode == "random_prefix" and prefix_k is not None:
            metrics["self_distillation/random_prefix_k"] = float(prefix_k)
            if demo_orig_token_lens:
                keep_fracs = [
                    (kept / orig) if orig > 0 else 0.0
                    for kept, orig in zip(demo_kept_token_lens, demo_orig_token_lens, strict=True)
                ]
                metrics["self_distillation/random_prefix_keep_frac_mean"] = float(np.mean(keep_fracs))
                metrics["self_distillation/random_prefix_kept_tokens_mean"] = float(np.mean(demo_kept_token_lens))
                metrics["self_distillation/random_prefix_orig_tokens_mean"] = float(np.mean(demo_orig_token_lens))
        elif demo_mode == "final_answer":
            metrics["self_distillation/final_answer_demo_fraction"] = num_with_solution / batch_size
        tensors = {
        ...
        capture_steps = set(diagnostics_cfg.get("capture_steps", []))
        if diagnostics_cfg.get("enable", False) and self.global_steps in capture_steps:
            full_prompt_lengths = []
            for message in messages:
                ...
                full_prompt_lengths.append(len(full_ids))
            prompt_lengths = teacher_prompt["attention_mask"].sum(-1).to(device)
            solution_token_lengths = [
                len(self.tokenizer.encode(solution, add_special_tokens=False)) if solution is not None else 0
                for solution in solution_strs
            ]
            sequence_rewards = reward_tensor.sum(-1)
            source_rewards = [
                sequence_rewards[idx].item() if idx >= 0 else 0.0
                for idx in solution_indices
            ]
            tensors.update(
                {
                    "diagnostic_teacher_prompt_length": prompt_lengths,
                    "diagnostic_teacher_prompt_full_length": torch.tensor(full_prompt_lengths, device=device),
                    "diagnostic_teacher_prompt_truncated": torch.tensor(
                        [length > self_distillation_cfg.max_reprompt_len for length in full_prompt_lengths],
                        dtype=torch.bool,
                        device=device,
                    ),
                    "diagnostic_solution_token_length": torch.tensor(solution_token_lengths, device=device),
                    "diagnostic_solution_source_index": torch.tensor(solution_indices, device=device),
                    "diagnostic_solution_source_reward": torch.tensor(source_rewards, device=device),
                    "diagnostic_sequence_reward": sequence_rewards.to(device),
                }
            )
        return DataProto.from_dict(tensors=tensors), metrics

    def _maybe_build_hybrid_routing_batch(
        self,
        batch: DataProto,
        reward_tensor: torch.Tensor,
    ) -> Optional[tuple[DataProto, dict[str, float]]]:
        offpolicy_cfg = self.config.algorithm.get("offpolicy_routing", {})
        far_sft_cfg = self.config.algorithm.get("far_correct_sft", {})
        sdpo_routing_cfg = self.config.algorithm.get("sdpo_routing", {})
        kl_routing_cfg = self.config.algorithm.get("kl_routing", {})
        needs_masks = (
            offpolicy_cfg.get("enable", False)
            or far_sft_cfg.get("enable", False)
            or sdpo_routing_cfg.get("enable", False)
            or kl_routing_cfg.get("enable", False)
        )
        if not needs_masks:
            return None

        sft_threshold = far_sft_cfg.get("correct_reward_threshold", 0.5)
        sdpo_threshold = sdpo_routing_cfg.get("correct_reward_threshold", 0.5)
        kl_threshold = kl_routing_cfg.get("correct_reward_threshold", 0.5)
        enabled_thresholds = []
        if far_sft_cfg.get("enable", False):
            enabled_thresholds.append(("far_correct_sft", sft_threshold))
        if sdpo_routing_cfg.get("enable", False):
            enabled_thresholds.append(("sdpo_routing", sdpo_threshold))
        if kl_routing_cfg.get("enable", False):
            enabled_thresholds.append(("kl_routing", kl_threshold))
        if len(enabled_thresholds) >= 2:
            base_name, base_thr = enabled_thresholds[0]
            for name, thr in enabled_thresholds[1:]:
                if thr != base_thr:
                    raise ValueError(
                        f"{base_name}.correct_reward_threshold and {name}.correct_reward_threshold "
                        "must match when both hybrid losses are enabled."
                    )
        success_threshold = enabled_thresholds[0][1] if enabled_thresholds else sdpo_threshold
        success_mask = compute_success_mask(reward_tensor, threshold=success_threshold)
        failed_mask = compute_failed_mask(success_mask)
        group_has_success = compute_group_has_success(success_mask, batch.non_tensor_batch["uid"])
        metrics = {
            "hybrid_routing/success_sample_fraction": success_mask.float().mean().item(),
            "hybrid_routing/failed_sample_fraction": failed_mask.float().mean().item(),
            "hybrid_routing/group_has_success_fraction": group_has_success.float().mean().item(),
        }
        return DataProto.from_dict(
            tensors={
                "hybrid_success_mask": success_mask.float(),
                "hybrid_failed_mask": failed_mask.float(),
                "hybrid_group_has_correct": group_has_success.float(),
            }
        ), metrics

    def _maybe_compute_hybrid_far_metrics(self, batch: DataProto) -> dict[str, float]:
        offpolicy_cfg = self.config.algorithm.get("offpolicy_routing", {})
        far_sft_cfg = self.config.algorithm.get("far_correct_sft", {})
        sdpo_routing_cfg = self.config.algorithm.get("sdpo_routing", {})
        kl_routing_cfg = self.config.algorithm.get("kl_routing", {})
        should_log = (
            offpolicy_cfg.get("log_metrics", True)
            or offpolicy_cfg.get("enable", False)
            or far_sft_cfg.get("enable", False)
            or sdpo_routing_cfg.get("enable", False)
            or kl_routing_cfg.get("enable", False)
        )
        if not should_log or "old_log_probs" not in batch.batch:
            return {}
        ...
        far_gate = compute_far_gate(
            ...
            tau_far=offpolicy_cfg.get("tau_far", 0.3),
            gate_temp=offpolicy_cfg.get("gate_temp", 0.1),
            use_soft_gate=offpolicy_cfg.get("use_soft_gate", True),
        )
        return {
            "hybrid_routing/full_batch_offpolicy_degree_mean": offpolicy_degree.mean().detach().item(),
            "hybrid_routing/full_batch_offpolicy_degree_max": offpolicy_degree.max().detach().item(),
            "hybrid_routing/full_batch_far_gate_mean": far_gate.mean().detach().item(),
            "hybrid_routing/full_batch_far_hard_fraction": (
                offpolicy_degree > offpolicy_cfg.get("tau_far", 0.3)
            ).float().mean().detach().item(),
            "hybrid_routing/full_batch_uses_rollout_log_probs": float(rollout_log_probs is not None),
        }
    ...
    def _debug_batch_input_sanity(self, batch: DataProto, label: str) -> None:
        if os.environ.get("SDPO_DEBUG_NAN", "0") != "1":
            return

        tensors = batch.batch
        if "input_ids" in tensors:
            input_ids = tensors["input_ids"]
            negative_count = (input_ids < 0).sum().item()
            ignore_count = (input_ids == -100).sum().item()
            print(
                "[SDPO_DEBUG_NAN] "
                f"{label}/input_ids: shape={tuple(input_ids.shape)} dtype={input_ids.dtype} "
                f"min={input_ids.min().item()} max={input_ids.max().item()} "
                f"negative={negative_count} ignore_index(-100)={ignore_count}",
                flush=True,
            )
            if negative_count > 0:
                bad_rows = torch.where((input_ids < 0).any(dim=1))[0][:8].tolist()
                print(
                    f"[SDPO_DEBUG_NAN] {label}/input_ids: rows_with_negative={bad_rows}",
                    flush=True,
                )

        if "attention_mask" in tensors:
            attention_mask = tensors["attention_mask"]
            row_sums = attention_mask.view(attention_mask.shape[0], -1).sum(dim=-1)
            zero_rows = torch.where(row_sums == 0)[0].tolist()
            print(
                "[SDPO_DEBUG_NAN] "
                f"{label}/attention_mask: shape={tuple(attention_mask.shape)} "
                f"row_sum_min={row_sums.min().item()} row_sum_max={row_sums.max().item()} "
                f"zero_row_count={len(zero_rows)} zero_rows={zero_rows[:8]}",
                flush=True,
            )

        if "response_mask" in tensors:
            response_mask = tensors["response_mask"]
            row_sums = response_mask.view(response_mask.shape[0], -1).sum(dim=-1)
            zero_rows = torch.where(row_sums == 0)[0].tolist()
            print(
                "[SDPO_DEBUG_NAN] "
                f"{label}/response_mask: row_sum_min={row_sums.min().item()} "
                f"row_sum_max={row_sums.max().item()} zero_row_count={len(zero_rows)}",
                flush=True,
            )

        if "position_ids" in tensors:
            position_ids = tensors["position_ids"]
            finite = torch.isfinite(position_ids)
            if finite.all():
                print(
                    "[SDPO_DEBUG_NAN] "
                    f"{label}/position_ids: shape={tuple(position_ids.shape)} "
                ...
                print(
                    "[SDPO_DEBUG_NAN] "
                    f"{label}/position_ids: non_finite="
                    f"{(~finite).sum().item()}/{position_ids.numel()}",
                    flush=True,
                )

        if "responses" in tensors:
            responses = tensors["responses"]
            zero_frac = (responses == 0).float().mean().item()
            print(
                "[SDPO_DEBUG_NAN] "
                f"{label}/responses: min={responses.min().item()} max={responses.max().item()} "
                f"zero_frac={zero_frac:.4f}",
                flush=True,
            )

    def _debug_tensor_finiteness(self, batch: DataProto, label: str, keys: tuple[str, ...]) -> None:
        if os.environ.get("SDPO_DEBUG_NAN", "0") != "1":
            return
        self._debug_batch_input_sanity(batch, label)
        for key in keys:
            if key not in batch.batch:
                print(f"[SDPO_DEBUG_NAN] {label}/{key}: missing", flush=True)
                continue
            value = batch.batch[key]
            if not torch.is_tensor(value):
                print(f"[SDPO_DEBUG_NAN] {label}/{key}: non_tensor={type(value)}", flush=True)
                continue
            finite = torch.isfinite(value)
            finite_count = finite.sum().item()
            total = value.numel()
            nan_count = torch.isnan(value).sum().item()
            posinf_count = torch.isposinf(value).sum().item()
            neginf_count = torch.isneginf(value).sum().item()
            if finite_count:
            ...
            print(
                "[SDPO_DEBUG_NAN] "
                f"{label}/{key}: shape={tuple(value.shape)} dtype={value.dtype} "
                f"finite={finite_count}/{total} nan={nan_count} +inf={posinf_count} -inf={neginf_count} "
                f"finite_min={min_value} finite_max={max_value} finite_mean={mean_value}",
                flush=True,
            )
    ...
    def _prepare_cspd_targets(self, batch: DataProto) -> tuple[DataProto, dict[str, float]]:
        ...
        tail_mode = str(loss_cfg.get("cspd_tail_mode", "residual"))
        ...
        target_metrics.update(
            _masked_diagnostic_stats(
                "diagnostics/cspd_successor_success", diagnostics["successor_success"], selected_candidates
            )
        )
        target_metrics.update(
            _masked_diagnostic_stats(
                "diagnostics/cspd_signed_candidate_advantage",
                diagnostics["signed_candidate_advantage"],
                selected_candidates,
            )
        )
        for name in (
            "behavior_tail_probability",
            "raw_tail_mass",
            "projected_tail_mass",
            "implied_tail_success",
            "posterior_mass",
            "state_to_mass_ratio",
        ):
            target_metrics.update(
                _masked_diagnostic_stats(f"diagnostics/cspd_{name}", diagnostics[name], valid)
            )
        if selected.any():
        ...
        return DataProto.from_dict(tensors={
            ...
            "cspd_value_weights": weights,
            "cspd_selected_mask": selected,
        }), target_metrics
    ...
    def _wall_clock_metrics(self) -> dict[str, float]:
        """Elapsed wall-clock time since fit() started (this process)."""
        start = getattr(self, "_train_wall_start", None)
        if start is None:
            self._train_wall_start = time.time()
            start = self._train_wall_start
        elapsed = time.time() - start
        return {
            "timing/wall_clock_sec": elapsed,
            "timing/wall_clock_min": elapsed / 60.0,
            "timing/wall_clock_hours": elapsed / 3600.0,
        }
    ...
    def fit(self):
        ...
        # Wall-clock since this fit() started (includes val/ckpt I/O in this process).
        # SwanLab UI: chart settings -> X-axis data source can switch step vs relative time.
        self._train_wall_start = time.time()
        ...
        if self.val_reward_fn is not None and self.config.trainer.get("val_before_train", True):
            ...
            val_metrics.update(self._wall_clock_metrics())
        ...
        for epoch in range(current_epoch, self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    metrics.update(self._maybe_compute_hybrid_far_metrics(batch))
                    ...
                    if self.use_critic:
                        ...
                        if self.config.actor_rollout_ref.actor.policy_loss.loss_mode == "cspd":
                            with marked_timer("cspd_targets", timing_raw, color="magenta"):
                                cspd_targets, cspd_target_metrics = self._prepare_cspd_targets(batch)
                                batch = batch.union(cspd_targets)
                            metrics.update(cspd_target_metrics)
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        hybrid_routing_data = self._maybe_build_hybrid_routing_batch(batch, reward_tensor)
                        if hybrid_routing_data is not None:
                            hybrid_routing_batch, hybrid_routing_metrics = hybrid_routing_data
                            batch = batch.union(hybrid_routing_batch)
                            metrics.update(hybrid_routing_metrics)
                        ...
                        if (
                            ...
                            self._debug_tensor_finiteness(
                                batch,
                                "before_rollout_correction",
                                ("old_log_probs", "rollout_log_probs", "response_mask"),
                            )
                        ...
                        raw_gae = batch.batch["returns"].float() - batch.batch["values"].float()
                        metrics.update(_masked_diagnostic_stats("diagnostics/raw_gae", raw_gae, response_mask))
                        raw_gae_valid = raw_gae.detach()[response_mask]
                        if raw_gae_valid.numel() > 0:
                            # Frozen full-rollout scalar used only to report the exact
                            # counterfactual norm for L_CSPD / std(raw GAE).
                            batch.meta_info["cspd_raw_gae_std"] = raw_gae_valid.std(unbiased=False).item()
                        metrics.update(
                            _masked_diagnostic_stats(
                                "diagnostics/whitened_gae", batch.batch["advantages"], response_mask
                            )
                        )
                ...
                metrics.update(self._wall_clock_metrics())
