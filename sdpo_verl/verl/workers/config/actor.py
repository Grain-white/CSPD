...
__all__ = [
    ...
    "SDPODiagnosticsConfig",
    "PolicyLossConfig",
    "OffpolicyRoutingConfig",
    "FarCorrectSFTConfig",
    "SDPORoutingConfig",
    "KLRoutingConfig",
    "RouterReplayConfig",
...
class SelfDistillationConfig(BaseConfig):
    ...
    Args:
        ...
        demonstration_content_mode (str): How to rewrite the sibling solution before templating.
            "full" keeps the (optionally think-stripped) solution; "random_prefix" keeps a random
            token prefix of length L~Uniform{{0,...,floor(k * n)}}; "final_answer" keeps only the
            last \\boxed{{...}} span (fallback: think-stripped text).
        random_prefix_k_start (float): Cosine-annealed k at step 0 for random_prefix mode (default 0.95).
        random_prefix_k_end (float): Cosine-annealed k at the final training step (default 0.05).
    ...
    demonstration_content_mode: str = "full"
    random_prefix_k_start: float = 0.95
    random_prefix_k_end: float = 0.05
    ...
    def __post_init__(self):
        ...
        valid_demo_modes = ["full", "random_prefix", "final_answer"]
        if self.demonstration_content_mode not in valid_demo_modes:
            raise ValueError(
                "self_distillation.demonstration_content_mode must be one of "
                f"{valid_demo_modes}, got {self.demonstration_content_mode}"
            )
        for name in ("random_prefix_k_start", "random_prefix_k_end"):
...
@dataclass
class SDPODiagnosticsConfig(BaseConfig):
    """Opt-in diagnostics for short SDPO/GRPO mechanism probes."""

    enable: bool = False
    output_dir: Optional[str] = None
    capture_steps: list[int] = field(default_factory=lambda: [0, 1, 5, 10, 15])
    max_prompt_groups: int = 8
    gradient_compare: bool = True
    gradient_sketch_dim: int = 2048
    capture_all_ppo_epochs: bool = True
    # Skip sketches when either objective has near-zero grad norm (common on math
    # when a microbatch has flat GRPO advantages). Keep scanning microbatches.
    gradient_min_norm: float = 1e-3
    # Collect up to this many *valid* gradient pairs per outer step (per rank).
    gradient_max_records_per_step: int = 4

    def __post_init__(self):
        if self.max_prompt_groups <= 0:
            raise ValueError("sdpo_diagnostics.max_prompt_groups must be positive")
        if self.gradient_sketch_dim <= 0:
            raise ValueError("sdpo_diagnostics.gradient_sketch_dim must be positive")
        if any(step < 0 for step in self.capture_steps):
            raise ValueError("sdpo_diagnostics.capture_steps must be non-negative")
        if self.gradient_min_norm < 0:
            raise ValueError("sdpo_diagnostics.gradient_min_norm must be non-negative")
        if self.gradient_max_records_per_step <= 0:
            raise ValueError("sdpo_diagnostics.gradient_max_records_per_step must be positive")
...
class PolicyLossConfig(BaseConfig):
    ...
    # no_tail: zero tail target and top-K posterior renormalization.
    cspd_tail_mode: str = "residual"


@dataclass
class OffpolicyRoutingConfig(BaseConfig):
    enable: bool = False
...
@dataclass
class FarCorrectSFTConfig(BaseConfig):
    enable: bool = False
...
@dataclass
class SDPORoutingConfig(BaseConfig):
    enable: bool = False
    mode: str = "none"
    correct_reward_threshold: float = 0.5
...
@dataclass
class KLRoutingConfig(BaseConfig):
    """Route ordinary KL with the same sample weights as M4 SDPO aux.

    Does not alter sdpo_routing. When enable=True, KL uses
    w = far_gate * failed * group_has_correct (via compute_sdpo_route_mask).
    """

    enable: bool = False
    mode: str = "none"
    correct_reward_threshold: float = 0.5
    # Prefactor on routed KL; M4 adds SDPO aux with implicit weight 1.0.
    lambda_kl: float = 1.0
...
class ActorConfig(BaseConfig):
    ...
    sdpo_diagnostics: SDPODiagnosticsConfig = field(default_factory=SDPODiagnosticsConfig)
    offpolicy_routing: OffpolicyRoutingConfig = field(default_factory=OffpolicyRoutingConfig)
    far_correct_sft: FarCorrectSFTConfig = field(default_factory=FarCorrectSFTConfig)
    sdpo_routing: SDPORoutingConfig = field(default_factory=SDPORoutingConfig)
    kl_routing: KLRoutingConfig = field(default_factory=KLRoutingConfig)
